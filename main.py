...
with st.expander('Enter Kata Kunci'):
    ...
    limit=st.text_input('Masukkan Limit')
    search_keyword = f"{kata_kunci} since:{e_since} until:{e_until}"
    ...
    if st.button("RUN", key="runnin") :
        ...
        command = f'npx tweet-harvest@latest -o "{filename}" -s "{search_keyword}" -l {limit} --token "$MY_API_TOKEN"'
